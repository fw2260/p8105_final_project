---
title: "Play with our game data!"
output: 
  flexdashboard::flex_dashboard:
    theme: cosmo
    orientation: columns
    vertical_layout: fill
    source: embed
runtime: shiny
---

<style>                     
.navbar {
  background-color:black;
  border-color:black;
}
</style> 

```{r setup, include=FALSE}
library(tidyverse)
library(plotly)
library(flexdashboard)
library(shiny)
```

```{r}
games_df = read_csv("./games.csv")
```

Column {.sidebar}
-----------------------------------------------------------------------

```{r sidebar}
developer_choices = c("All", unique(sort(games_df$console_developer)))
radioButtons(
  "developer_choice",
  h3("Console Developer"),
  choices = developer_choices, selected = "All"
)

genre_choices = c("All", unique(sort(games_df$genre)))
selectInput(
  "genre_choice",
  h3("Genre Choice"),
  choices = genre_choices,
  selected = "All"
)

sliderInput(
  "year_range",
  h3("Year Range"),
  2001, 2020, 
  value = c(2001, 2020),
  ticks = FALSE,
  sep = "")
```


Column {data-width=650}
-----------------------------------------------------------------------
### Top 10 video games per developer

```{r}
top_10_df = 
  games_df %>% 
  group_by(console_developer) %>% 
  top_n(10, total_sale)


sub_games_10_df = reactive({
  
  if (input$developer_choice == "All") {
    if (input$genre_choice == "All"){
      top_10_df
    } else {
      top_10_df %>% filter(genre == input$genre_choice)
    }
  } else {
    if(input$genre_choice == "All") {
      top_10_df %>% filter(console_developer == input$developer_choice)
    } else {
      top_10_df %>% 
        filter(genre == input$genre_choice,
               console_developer == input$developer_choice)
    }
  }
})

renderPlotly({
  sub_games_10_df() %>% 
    filter(
      release_year %in% input$year_range[1]:input$year_range[2] 
    ) %>% 
    plot_ly(x = ~title, y = ~total_sale, type = "bar", color = ~console) %>%
    layout(
      xaxis = list(
        title = "Game title",
        titlefont = list(
          family = "Courier New, monospace",
          size = 13,
          color = "#7f7f7f"
        )),
      yaxis = list(
        title = "Total Sale (million USD)",
        titlefont = list(
          family = "Courier New, monospace",
          size = 13,
          color = "#7f7f7f"
        )),
      showlegend = FALSE)
})

```

Column {data-width=550}
-----------------------------------------------------------------------

### Annual global video game market per console

```{r}
console_df = 
  games_df %>% 
  group_by(console, genre, console_developer, release_year) %>% 
  mutate(sum_console = sum(total_sale))

sub_console_df = reactive({
  
  if (input$developer_choice == "All") {
    if (input$genre_choice == "All"){
      console_df
    } else {
      console_df %>% filter(genre == input$genre_choice)
    }
  } else {
    if(input$genre_choice == "All") {
      console_df %>% filter(console_developer == input$developer_choice)
    } else {
    console_df %>% 
      filter(genre == input$genre_choice,
             console_developer == input$developer_choice)
    }
  }
})

renderPlotly({
  sub_console_df() %>% 
    filter(
       release_year %in% input$year_range[1]:input$year_range[2] 
    ) %>% 
    plot_ly(x = ~sum_console, y = ~console, color = ~console, type = "scatter") %>% 
    layout(
      xaxis = list(
        title = "Console",
        titlefont = list(
          family = "Courier New, monospace",
          size = 16,
          color = "#7f7f7f"
        )),
      yaxis = list(
        title = "Total Sale (million USD)",
        titlefont = list(
          family = "Courier New, monospace",
          size = 16,
          color = "#7f7f7f"
        )),
      showlegend = FALSE)
})
```

### Total sales over time

```{r}
sub_games_df = reactive({
  
  if (input$developer_choice == "All") {
    if (input$genre_choice == "All"){
      games_df
    } else {
      games_df %>% filter(genre == input$genre_choice)
    }
  } else {
    if(input$genre_choice == "All") {
      games_df %>% filter(console_developer == input$developer_choice)
    } else {
      games_df %>% 
      filter(genre == input$genre_choice,
             console_developer == input$developer_choice)
    }
  }
})

renderPlotly({
  sub_games_df() %>%
    group_by(release_year) %>% 
    filter(release_year %in% input$year_range[1]:input$year_range[2]) %>%
    summarize(totalsale = sum(total_sale),
              count = n()) %>%
    mutate(text_label = str_c("Number of games: ", count)) %>%
    plot_ly(x = ~release_year, y = ~totalsale, type = "scatter", mode = "lines", text = ~text_label) %>% 
    layout(
      xaxis = list(
        title = "Year",
        titlefont = list(
          family = "Courier New, monospace",
          size = 16,
          color = "#7f7f7f"
        )),
      yaxis = list(
        title = "Total Sale (million USD)",
        titlefont = list(
          family = "Courier New, monospace",
          size = 16,
          color = "#7f7f7f"
        )))

})
```