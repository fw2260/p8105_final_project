...
Video game industry increased at a rapid rate after 2001 with drastic increase in number of game released each year and the high volume reached its peak in 2009, with 587 games published on these major platforms. Presently, games generating the biggest revenue are also those released in 2009, like Call of Duty: Modern Warfare 2 on Xbox 360. Cumulative sales for games which came after 2009 decreased over year. It's quite intuitive because they are younger than their predecessor. And also, as the hardware lifecycle progressed, the sales of games went down. Also, there was a drop in the number of games released on these major consoles after 2009, especially after 2018. Apart from unavailability of sales information, it's also because of the higher expense to produce game consoles. Nowadays, hardware makers are not eager to release new models every five years, which also decreased the number of games published each year.
...
By Analysis of Variance (ANOVA) we can conduct comparisons among groups of our interested categorical variables : n_platforms and genre.In the test of n_platforms, we want to answer the question: Whether there are true differences in games sales among different groups of number of platforms? 

We divided n_platform into 3 groups (criteria of grouping: 1 ~ small, 2-3 ~ medium, 4-9 ~ large). 
...
### 2. ANOVA with interaction: Total_sale ~ console_developer * n_platforms 

```{r }
aov_mod_2 = aov(total_sale ~ console_developer * n_platforms, data = games_df)
```
...
```
 ...
 With p.value of the interaction term = < 0.001, we conclude that at least two of the interaction combinations have different average total_sales.From Tukey’s multiple comparisons we get that there are `r nrow(multi_df)` significant different combinations among `r nrow(ori_df)` combinations of console_developer and n_platforms.
 
<br>

### 3. Conclusion

   Anova tests above provide us evidence that indicate there are true differences in games sales among different groups of genre. When the console_developer changes, significant differences exist in subgroups as well.  Thus it’s reasonable to include the interaction term console_developer * n_platforms in the linear regression model.
  
  Following similar procedures, we also conclude that significant differences exist in groups of genres and subgroups considering interaction with console_developer. Therefore we include the interaction term console_developer * genre in the linear regression model 
