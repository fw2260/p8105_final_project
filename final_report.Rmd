...
We divided n_platform into 3 groups (criteria of grouping: 1 ~ small, 2-3 ~ medium, 4-9 ~ large) and further narrow down the number of genre from 13 to 6, according to their similarity.
...
Model 4:  total_sale ~ console_developer * n_platforms + console_developer * critic_score
   ...
   We set the cutoff value at 5 (million USD). Model 2 contains interaction between console_developer and all pther interested predictors (genre, n_platform, critic_score) while Model 1,3,4 contain interaction between console_developer and only two of interested predictors. 
...
Based on these results, there's minor improvement for adding `console_developer*critic_score` as an additional predictor. Maybe Model 2 is a little bit redundant compared to Model 3 and 4, with which it have similar **RMSE** values, but it can provide information on most of our initial questions. In this way, we select Model 2 as our final model.
