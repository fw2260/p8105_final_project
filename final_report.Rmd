...
The video game industry increased at a rapid rate after 2001 with a drastic increase in number of games released each year and the high volume reached its peak in 2008, with 620 games published on these major platforms. After 2008, both cumulative sales and number of games released decreases over time.  
...
Based on the boxplot of total sales, the values of total sales are heavily skewed right. In order to cut off the outliers, the dataset was filtered to only include values less than 5. Therefore any values greater than 5 are excluded for linear regression analysis. 
...
From the analysis above, **2008** is the year with the **highest global revenues** and the **most games released**. In 2008, many popular games with high revenues are released in gaming industry such as *Grand Theft Auto IV* and *Mario Kart Wii*. After 2008, both cumulative sales and number of games released decrease due the effects of the 2008 financial recession and the rise in popularity of PC and mobile gaming as technology advances. Another possible reason for the downfall is that the console makers are not eager to release new models as often, which also decreases the number of games published each year. Furthermore, sales of more recently released games may be lower because they are younger than their predecessors. 
...
In order to validate the results of our main findings, *ANOVA test* and *linear regression* are performed. Our statistical analyses confirmed that the critic scores and the number of available platforms per game play big factors in influencing the global revenues of each game. 
...
There are still some limitation of our dataset. 1. The regression model only includes games with total sale less then 5 million USD and omit those greatest games which would be significant outlier in the linear regression model. Also, we build models by hypothesize instead of performing a model selection procedure such as stepwise method to find the 'best' model. The resulting model doesn't have a ideal distribution of residuals, the r.square is quite low as well which indicate the model should be improved. We also find hard interpreting a large model while containing all variables in it since there are too many categorical variables to concern. 
<br>
...
Video game preferences tend to shift regularly, but a look at the map of the top 10 best-selling games of the last 19 years shows how loyal players have remained to some of the industry's biggest franchises. 
The bar graph shows the top 10 best-selling video games based on the selection of company, genre, and release year range. Overall, from 2001 to 2020, Nintendo holds the top place of total revenues with *Wii Sports* seem to dominate the chart with more than 80 million USD. 

### 2. Distribution of total sales by console

The second chart is a boxplot showing the distribution of total sales by console. Still regardless of developer, genre, and release year range, the second chart implies that the distribution of total sales is approximately normal, with center at around 0.5 million USD or 500,000 USD. 

### 3. Total sales by release year

The third chart gives the total sales of the release year. The highest total sales of the release year range occur in 2008. 
